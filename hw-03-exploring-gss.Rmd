...
***The possible responses are "Does not apply" (96), "No" (1136), "Yes" (237), and "N/A" (1398).***
...
***17% of respondents were harassed by their superiors or co-workers at their job.***
...
***The median is a better measure of typical time Americans spend on email. It is closer to the most frequent times reported.***
...
```{r}
gss16 <- gss16 %>%
  mutate(snap_insta = ifelse(snapchat == "Yes" | instagrm == "Yes", "Yes", 
                             ifelse(snapchat == "no" & instagrm == "no", "No",
                             ifelse(is.na(snapchat) & is.na(instagrm), NA_character_, "No"))))
...
***The percentage of Yes answers is 37.4%.***
...
***The possible responses are: Keeping house (284), Other (89), Retired (574), School (76), Temp not working (57), Unempl, laid off (118), Working fulltime (1321), Working parttime (345), and NA (3).***
...
email_probs <- lm(email ~ educ + wrkstat + snap_insta, data = gss16) %>%
  tidy()

email_probs

```
***For each additional year of education, email minutes increase. For the work status (Other), (Retired), (Unemployed), (Fulltime), and (parttime), email minutes are higher. For people who use snapchat or instagram, email minutes increase.***
...
predictions <- predict(email_probs)
residuals <- residuals(email_probs)
...
**Note:** The levels of this variables are spelled inconsistently: "Extremely liberal" vs. "Extrmly conservative".
...
gss16 <- gss16 %>%
  mutate(science_dichotomy = case_when(
...
gss16 <- gss16 %>%
  mutate(pols_dichotomy = case_when(
...
sci_vs_pols <- linear_reg() %>%
  set_engine("lm") %>%
  fit(science_dichotomy~pols_dichotomy, data = gss16) %>%
  tidy()

sci_vs_pols

```
*For some reason this code gives an error:* 
***Error in fit(., science_dichotomy ~ pols_dichotomy, data = gss16) : 
could not find function "fit"***
*which I could not resolve, but I'm sure that this is the first step.*
